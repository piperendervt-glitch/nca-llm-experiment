"""
run_experiment_nca_v3.py

NCA v3（Devil's Advocate + Balanced Prompts）実験。
Fixed, v1, v2 は既存結果を流用し、NCA v3 のみ新規実行する。
"""
...
from nca_network_v3 import run_nca_network
...
def run_nca_v3_experiment(tasks: list) -> list:
    """実験群: NCA v3 で全タスクを実行。"""
    results = []
    correct_count = 0
    ...
    existing_count = 0
    if NCA_V3_RESULTS_PATH.exists():
        with open(NCA_V3_RESULTS_PATH, "r", encoding="utf-8") as f:
            for line in f:
    ...
    print("\n" + "=" * 60)
    print("実験群 (NCA v3 Network) 実行開始")
    print("=" * 60)

    mode = "a" if existing_count > 0 else "w"
    with open(NCA_V3_RESULTS_PATH, mode, encoding="utf-8") as f:
...
def print_summary(results_fixed, results_v3):
    """4way比較サマリーを表示する。"""
    ...
    for label_name, label_val in [("CONSISTENT", True), ("CONTRADICTION", False)]:
        ...
        print(f"  Fixed: {acc_f:.1%} | NCA v1: {acc_1:.1%} | NCA v2: {acc_2:.1%} | NCA v3: {acc_3:.1%}")

    # Groupthink stats for v3
    all_contradiction = 0
    all_consistent = 0
    split = 0
    for r in results_v3:
...
def main():
    ...
    # NCA v3 を実行
    results_v3 = run_nca_v3_experiment(tasks)

    # 4way比較サマリー
    print_summary(results_fixed, results_v3)

    print(f"\n結果ファイル:")
    print(f"  {NCA_V3_RESULTS_PATH}")
